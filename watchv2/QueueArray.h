...
void QueueArray<T>::resize (const int s) {
  // defensive issue.
  if (s <= 0)
      exit (PSTR("QUEUE: error due to undesirable size for queue size."));
  ...
  // if there is a memory allocation error.
  if (temp == NULL)
      exit (PSTR("QUEUE: insufficient memory to initialize temporary queue."));
...
T QueueArray<T>::dequeue () {
  // check if the queue is empty.
  if (isEmpty ())
      exit (PSTR("QUEUE: can't pop item from queue: queue is empty."));
...
T QueueArray<T>::front () const {
  // check if the queue is empty.
  if (isEmpty ())
      exit (PSTR("QUEUE: can't get the front item of queue: queue is empty."));
...
template<typename T>
    void QueueArray<T>::toArray(T *arr, int n) const {
    for (int i = 0; i < n; ++i) {
        arr[i] = contents[i];
    }
}
